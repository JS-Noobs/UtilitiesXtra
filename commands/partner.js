...
module.exports = {
  ...
  execute: async (message, args, client) => {
    ...
    if(args[0] === `requirements`) {
      ...
      const daysNeeded = botJoined >= botAge ? '✅' : ms(botAge - botJoined, {verbose: true}) + ' left until reached'
      const embed = new MessageEmbed()
      .setTitle('Partnership requirements')
      .addField(`${members} or more members`, membersNeed, true)
      .addField(`Used bot for ${ms(botAge - botJoined, {verbose: true})}`, daysNeeded, true)
